...
def makewindow():
    ...
    cmds.window(winName, w=500, h=310, s=False)

    # Create a form layout for the UI elements
    form = cmds.formLayout()

    # Title text at the top
    title = cmds.text(label="Maya UI Changer", align="center", height=30)

    # Define themes and their respective icon paths
    themes = ["Blender Dark", "Blender Light", "Edgerunners"]
    icon_paths = {
    ...
    for i, theme in enumerate(themes):
        ...
        # Position buttons in the form layout
        if i == 0:
            cmds.formLayout(form, edit=True, attachForm=[
                (title, 'top', 10), (title, 'left', 10), (title, 'right', 10),
                (button, 'left', 10), (button, 'top', 50)
            ])
        else:
            cmds.formLayout(form, edit=True, attachForm=[
                (button, 'top', 50)
            ], attachControl=[
                (button, 'left', 10, buttons[i - 1])
            ])

    # Highlight on hover for buttons (using PySide2)
...
def create_menu():
    ...
    cmds.menuItem(label='Blender Dark', command=lambda t='Blender Dark': on_theme_change(t))
    cmds.menuItem(label='Blender Light', command=lambda t='Blender Light': on_theme_change(t))
    cmds.setParent('..', menu=True)
    
    # Add new menu items
    cmds.menuItem(label='Edgerunners', command=lambda t='Edgerunners': on_theme_change(t))
    cmds.menuItem(label='Maya Default', command=lambda t='Maya Default': on_theme_change(t))
    ...
    if isinstance(widget, QtWidgets.QWidget):
        if selected_object == "Blender Dark":
            ...
            widget.setStyleSheet("""
                ...
                /*-----QPushButton------------------------------------------------------------------------------------------------------------------------------------*/           
                QPushButton { color: rgb(29, 237, 131); background-color: transparent; border: 1px solid rgb(94,246,255); border-radius: 0.3em;  }
